import { UserFlags, Permissions, MessageActionRow, MessageButton } from 'discord.js';
import { emojis, botColor, imgOpts } from '../defaults.js';
import { toUTS, flagToEmoji, collMap, monthDiff } from '../utils.js';
...
export async function execute(client, interaction, st, embed) {
  ...
  if (interaction.isApplicationCommand()) {
    ...
    if (commandName === 'User Info' || options?.getSubcommand(true) === 'info') {
      ...
      if (userO.id === guild?.ownerId) {
        flags.push(emojis.serverOwner);
      }
      ...
      if (memberO) {
        ...
        if (mRoles.size > 0) {
          embs[0].addField(`${st.__('GENERIC.ROLES')} [${mRoles.size}]`, collMap(mRoles));
        }
      }
